...
| `403 Access to unscoped api is not allowed` | Not a role. The OAuth client is refused the API at the gate: its Application Registry entry is **Securely Scoped** and has no **REST API Auth Scope** linked for `POST` on this API. Link one covering both `/api/now/identifyreconcile` and `/api/now/identifyreconcile/query`, or set **Scope Restriction = Broadly Scoped**. Whether `SNOW_WRITE_MODE=cmdb_instance` is refused too is instance-specific — it is a separate API with its own auth scope, and has been observed **allowed** on an instance that refuses `identifyreconcile`, which makes it a real fallback rather than a workaround for the same block. Run `--check-api` to find out rather than assuming either way. `SNOW_AUTH_MODE=basic` sidesteps the gate entirely for local testing, because the restriction is on the OAuth entity rather than the user. Reads keep working throughout, so a run report with `users_resolved > 0` and every device in error is the signature. |
| `Invalid data source` / `INVALID_INPUT_DATA - In payload invalid data source [X] exist` | Step 5 was skipped, or `SNOW_DISCOVERY_SOURCE` does not match the choice value exactly (case and spacing included). `--check` reports this before any write and names registered values resembling yours. `intune-cmdb-sync --register-discovery-source` creates the choice value itself where the credential is allowed to write `sys_choice`; where it is not, it prints the exact record for an admin. |
