...
class TestIreErrorReporting:
    """`/api/now/identifyreconcile` normally reports per-item failures inside a
    200, which `_parse_results` handles. A 4xx can still carry the same envelope
    -- the CMDB Instance endpoint runs through the same engine and was observed
    doing exactly that live on 2026-09-04 -- and there the message sits past the
    truncation point of the raw-body snippet."""

    @respx.mock
    def test_the_structured_error_survives_a_4xx(self, snow_client, config: Config):
        respx.post(IRE).mock(return_value=httpx.Response(400, json=INVALID_DATA_SOURCE))
        ...
        # The whole sentence, including the table the field lives on.
        assert "in table [cmdb_ci]" in message
        assert "identificationAttempts" not in message

    @respx.mock
    def test_a_body_that_is_not_an_ire_envelope_still_reports(
        self, snow_client, config: Config
    ):
        respx.post(IRE).mock(return_value=httpx.Response(400, text="Bad Request"))
        with pytest.raises(ServiceNowError, match="Bad Request"):
            IdentifyReconcileWriter(snow_client, config.servicenow).write([payload()])
